...
void dlgRoomExits::slot_editSpecialExit(QTreeWidgetItem * pI, int column ) {
    ...
    if ( mpEditItem != 0 && ( pI != mpEditItem || mEditColumn != column ) ) {
        ...
        mpEditItem = 0;   //This will cause a new PE to be opened, it will also be zeroed on the first time this function is called
        mEditColumn = -1;
    }
...
// FIXME: erasing the exit command will not work to delete an exit, indeed if
// it is changed this new code will fail to remove the exit with the unmodified
// command.  Needs to be redone to iterate though "originalSpecialExits"...
...
//    pR->clearSpecialExits(); // Previous code could not change the destination
    ...
    for ( int i=0; i<specialExits->topLevelItemCount(); i++ ) {
        ...
        if ( value != "<command or Lua script>" ) {
            if ( pI->checkState(1) == Qt::Unchecked )
                value = value.prepend( '0' );
            else
                value = value.prepend( '1' );

            if ( key > 0 ) { // We do want a special exit
                if ( mpHost->mpMap->mpRoomDB->getRoom(key) !=0 ) { // And the destination exists
                    pR->addSpecialExit( key, value );
                    if ( pR->hasExitWeight(value.mid(1))  || weight > 0 )
                        pR->setExitWeight(value.mid(1), weight);
                    if ( pR->getDoor(value.mid(1)) > 0 || door > 0 )
                        pR->setDoor(value.mid(1), door);
                } else { // The destination does NOT exist
                    if( pR->hasSpecialExit(value.mid(1)) )
                        pR->removeSpecialExit(value.mid(1));
                        // If we had that special exit command we need to zap it
                    pR->setDoor(value.mid(1), 0); // zap any doors
                    pR->setExitWeight(value.mid(1), 0); // zap any explicit weighting
                    if( pR->customLines.contains(value.mid(1)) )
                        pR->customLines.remove(value.mid(1));
                    if( pR->customLinesArrow.contains(value.mid(1)) )
                        pR->customLinesArrow.remove(value.mid(1));
                    if( pR->customLinesStyle.contains(value.mid(1)) )
                        pR->customLinesStyle.remove(value.mid(1));
                    if( pR->customLinesColor.contains(value.mid(1)) )
                        pR->customLinesColor.remove(value.mid(1));
                }
            } else { // We do not want this destination so zap anything related to the command
                if( pR->hasSpecialExit(value) )
                    pR->removeSpecialExit(value);
                    // If we had that special exit command we need to zap it
                pR->setDoor(value.mid(1), 0); // zap any doors
                pR->setExitWeight(value.mid(1), 0); // zap any explicit weighting
                if( pR->customLines.contains(value.mid(1)) )
                ...
                if( pR->customLinesColor.contains(value.mid(1)) )
                    pR->customLinesColor.remove(value.mid(1));
            } // End of Else (NOT) got a positive toId
        } // End of If got an exit command
    }

    if (nw->isEnabled() && nw->text().size() > 0 && mpHost->mpMap->mpRoomDB->getRoom(nw->text().toInt()) != 0 ) { // There IS a valid exit on the dialogue in this direction
        pR->setNorthwest(nw->text().toInt()); // So store it
        ...
        pR->setNorthwest(-1); // So ensure the value for no exit is stored
    ...
    if (out->isEnabled() && out->text().size() > 0 && mpHost->mpMap->mpRoomDB->getRoom(out->text().toInt()) != 0 ) {
        ...
        pR->setExitWeight( "out", 0);
        if( pR->customLines.contains( "OUT" ) )
            pR->customLines.remove( "OUT" );
        if( pR->customLinesArrow.contains( "OUT" ) )
            pR->customLinesArrow.remove( "OUT" );
        if( pR->customLinesStyle.contains( "OUT" ) )
            pR->customLinesStyle.remove( "OUT" );
        if( pR->customLinesColor.contains( "OUT") )
    ...
    TArea * pA = mpHost->mpMap->mpRoomDB->getArea( pR->getAreaId() );
    if( pA )
        pA->fast_ausgaengeBestimmen( pR->getId() );  // The "fast_" version CAN handle exits REMOVAL
...
void dlgRoomExits::slot_nw_textEdited(const QString &text) {
    ...
    if ( exitToRoom != 0 ) {
        ...
        if( exitToRoom->name.trimmed().length() )
            toolTipText = "Exits to \"" % exitToRoom->name % "\"";
        else
            toolTipText = "Valid exit!";
...
void dlgRoomExits::initExit( int roomId, int direction, int exitId, QLineEdit * exitLineEdit,
    ...
    if ( exitId > 0 ) { //Does this exit point anywhere
        ...
        if( mpHost->mpMap->mpRoomDB->getRoom( exitId )->name.trimmed().length() )
            exitLineEdit->setToolTip( "Exits to \"" % mpHost->mpMap->mpRoomDB->getRoom( exitId )->name % "\"" );
        else
            exitLineEdit->setToolTip( "Valid exit!" );
        ...
        if ( pR->hasExitStub( direction ) ) {
            ...
            exitLineEdit->setToolTip("Set the number of the room northwest of this one, will be blue for a valid number or red for invalid.");
...
void dlgRoomExits::init( int id ) {
    ...
    if( pR->name.trimmed().length() )
        ...
        titleText.append(QString("Id(%1) [*]").arg( id ));

    this->setWindowTitle(titleText);
    this->setWindowIcon(QIcon("://icons/mudlet_room_exits.png"));
    ...
    while ( it.hasNext() ) {
        ...
        pI->setCheckState( 6, Qt::Unchecked );
        {
            ...
            case 3:
                ...
                break;
            default:
                qDebug("dlgRoomExits::init() unexpected (other exit) doors \"%s\" value:%i found for roomID(%i)!", qPrintable( dir ), specialDoor, id);
            }
            originalSpecialExits.value( dir )->door = specialDoor;
        }

        //7 is new, but holds the command/script that was in 2
        pI->setText( 7, dir );
